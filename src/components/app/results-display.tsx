...
export function ResultsDisplay({ results, calculationMode }: ResultsDisplayProps) {
    const { onPremTCO, cloudTCO, savings, yearlyCosts, analysisPeriod, breakevenPoint } = results;

    const winningOption = useMemo(() => (savings > 0 ? 'Cloud' : 'On-Premise'), [savings]);
    const winningSavings = useMemo(() => (savings > 0 ? savings : -savings), [savings]);
    
    const onPremDisplay = useMemo(() => (calculationMode === 'amortized' ? onPremTCO / analysisPeriod : onPremTCO), [calculationMode, onPremTCO, analysisPeriod]);
    const cloudDisplay = useMemo(() => (calculationMode === 'amortized' ? cloudTCO / analysisPeriod : cloudTCO), [calculationMode, cloudTCO, analysisPeriod]);
    const savingsDisplay = useMemo(() => (calculationMode === 'amortized' ? winningSavings / analysisPeriod : winningSavings), [calculationMode, winningSavings, analysisPeriod]);

    const displayTitle = calculationMode === 'amortized' ? `Amortized Cost Per Year over ${analysisPeriod} years` : `Total Cost of Ownership (TCO) over ${analysisPeriod} years`;
        ...
        <div className="flex justify-between items-start">
            <div>
                ...
                <CardDescription>
                 ...
                 {breakevenPoint && <p className="text-sm text-muted-foreground mt-2">Breakeven Point: {breakevenPoint}</p>}
            </div>
      ...
      <CardContent className="space-y-6">
        ...
        <Tabs defaultValue="chart">
            ...
            <TabsContent value="chart" className="pt-4">
                 <h3 className="text-lg font-semibold mb-4 font-headline">Annual Cost Breakdown</h3>
            ...
            <TabsContent value="table" className="pt-4">
                <h3 className="text-lg font-semibold mb-4 font-headline">Yearly TCO Breakdown</h3>
                <ResultsTable data={yearlyCosts} />
            </TabsContent>
            <TabsContent value="breakdown" className="pt-4">
                <h3 className="text-lg font-semibold mb-4 font-headline">Detailed Cost Breakdown</h3>
