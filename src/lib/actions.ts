...
import { CostFormSchema, type CalculationResult, type CostFormValues, type YearlyCost, type CostBreakdown } from '@/lib/types';
...
export async function calculateCosts(
  ...
  try {
    ...
    for (let year = 1; year <= analysisPeriod; year++) {
      ...
      const currentCloudEgressGb = unitToGb(currentCloudEgressUnits, 'TB') * 1024; // Egress cost is per GB

      const hotStorageGb = currentCloudStorageGb * (cloudHotTier / 100);
      const standardStorageGb = currentCloudStorageGb * (cloudStandardTier / 100);
      ...
      const [inflatedStorage, inflatedEgress] = await Promise.all([
          modelInflation({ initialCost: baseCloudStorageCost, inflationRate: inflationDecimal, analysisPeriod: year - 1 }),
          modelInflation({ initialCost: baseCloudEgressCost, inflationRate: inflationDecimal, analysisPeriod: year - 1 })
      ]);
      totalAnnualCloudCost = inflatedStorage.inflatedCost + inflatedEgress.inflatedCost;
      cloudBreakdown['Hot Storage'] = inflatedStorage.inflatedCost * (hotCost / baseCloudStorageCost);
      cloudBreakdown['Standard Storage'] = inflatedStorage.inflatedCost * (standardCost / baseCloudStorageCost);
      cloudBreakdown['Archive Storage'] = inflatedStorage.inflatedCost * (archiveCost / baseCloudStorageCost);
      ...
      if (calculationMode === 'tco') {
        ...
        if (year === analysisPeriod) salvageCredit = -salvageValueAmount;
         ...
         const { inflatedCost } = await modelInflation({ initialCost: onPremHardwareCost / analysisPeriod, inflationRate: inflationDecimal, analysisPeriod: year - 1 });
         hardwareCost = inflatedCost;
         const { inflatedCost: inflatedSalvage } = await modelInflation({ initialCost: salvageValueAmount / analysisPeriod, inflationRate: inflationDecimal, analysisPeriod: year - 1 });
         salvageCredit = -inflatedSalvage;
      }

      let softwareCost = onPremYearlyLicensingCost;
      
      let powerCost = ((onPremPowerRating * (onPremLoadFactor / 100) * 24 * 365) / 1000) * onPremElectricityCost;
      
      let driveReplacementCost = (onPremTotalDrives * (onPremDriveFailureRate / 100)) * onPremDriveReplacementCost;
      
      let bandwidthCost = (currentOnPremBandwidthGb * onPremBandwidthCostPerGb);
      let cdnCost = useOnPremCdn ? (currentOnPremCdnUsageGb * onPremCdnCostPerGb) : 0;

      let backupCost = 0;
      if (useOnPremBackup && onPremBackupStorage && onPremBackupCostPerUnit) {
        const backupCostPerUnitInGb = onPremBackupCostPerUnit / { 'GB': 1, 'TB': 1024, 'PB': 1024 * 1024 }[dataUnit];
        const backupStorageInGb = unitToGb(onPremBackupStorage, dataUnit);
        backupCost = backupStorageInGb * backupCostPerUnitInGb;
      }
      
      const replicationFactor = (useOnPremReplication && onPremReplicationFactor) ? onPremReplicationFactor : 0;
      
      // Inflate all recurring costs
      const recurringCosts = { softwareCost, powerCost, driveReplacementCost, bandwidthCost, cdnCost, backupCost };
      ...
      for (const key in recurringCosts) {
          ...
          if (cost > 0) {
              const { inflatedCost } = await modelInflation({ initialCost: cost, inflationRate: inflationDecimal, analysisPeriod: year - 1 });
      ...
      onPremBreakdown['Hardware'] = hardwareCost + inflatedRecurring.driveReplacementCost;
      ...
      let totalAnnualOnPremCost = hardwareCost + salvageCredit + inflatedRecurring.driveReplacementCost;

      const replicatedCosts = [inflatedRecurring.softwareCost, inflatedRecurring.powerCost, inflatedRecurring.bandwidthCost, inflatedRecurring.cdnCost];
      ...
      if (replicationFactor > 0) {
        ...
        const singleSiteRecurring = inflatedRecurring.softwareCost + inflatedRecurring.powerCost + inflatedRecurring.bandwidthCost + inflatedRecurring.cdnCost + inflatedRecurring.driveReplacementCost;
        let singleSiteHardware = 0;
        if(calculationMode === 'tco' && year === 1) singleSiteHardware = onPremHardwareCost;
        ...
        onPremBreakdown['Software'] += inflatedRecurring.softwareCost * replicationFactor;
        onPremBreakdown['Power'] += inflatedRecurring.powerCost * replicationFactor;
        onPremBreakdown['Bandwidth'] += inflatedRecurring.bandwidthCost * replicationFactor;
        onPremBreakdown['CDN'] += inflatedRecurring.cdnCost * replicationFactor;
      }
      
      totalAnnualOnPremCost = Object.values(onPremBreakdown).reduce((sum, val) => sum + (val || 0), 0);
    ...
    const initialOnPremIsCheaper = yearlyData[0].cumulativeOnPrem < yearlyData[0].cumulativeCloud;
    ...
    if (initialOnPremIsCheaper !== finalOnPremIsCheaper) {
      for (let i = 0; i < yearlyData.length; i++) {
        ...
        const prevYearData = i > 0 ? yearlyData[i-1] : { cumulativeOnPrem: 0, cumulativeCloud: 0 };
        
        const prevOnPremWasCheaper = prevYearData.cumulativeOnPrem < prevYearData.cumulativeCloud;
        const currentOnPremIsCheaper = currentYearData.cumulativeOnPrem < currentYearData.cumulativeCloud;

        if (prevOnPremWasCheaper !== currentOnPremIsCheaper) {
            const costDiffAtStartOfYear = prevYearData.cumulativeOnPrem - prevYearData.cumulativeCloud;
            const costDiffAtEndOfYear = currentYearData.cumulativeOnPrem - currentYearData.cumulativeCloud;
