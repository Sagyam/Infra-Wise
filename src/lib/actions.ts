...
> {
  try {
    ...
    const onPremTrafficGrowthFactor =
      1 + (onPremAnnualTrafficGrowth || 0) / 100;
    ...
    for (let year = 1; year <= analysisPeriod; year++) {
      ...
      // --- Cloud Costs ---
      const currentCloudStorageGb = unitToGb(
        currentCloudStorageUnits,
        dataUnit
      );
        ...
        if (prevOnPremWasCheaper !== currentOnPremIsCheaper) {
          ...
          if (totalChangeInDiff !== 0) {
            const crossoverFraction =
              -costDiffAtStartOfYear / totalChangeInDiff;
